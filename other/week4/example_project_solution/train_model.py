    ...
    for artist in os.listdir(SONG_PATH):
        ...
        for filename in tqdm(os.listdir(path)):
            if filename.endswith('.html'):
                ...
                try:
                    section = song.find_all(attrs={'class':re.compile(r'lyric-body.*')})[0]
                    songs.append(extract_lyrics(section))
...
def train_model(X, y):
    ...
    pipeline = make_pipeline(
        ...
        MultinomialNB(alpha=1.5) #TODO: hyperparameter search
    )
    pipeline.fit(X, y)
